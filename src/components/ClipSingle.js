    ...
    const getClips = async (channelID, clipDate) => {
        try {
          const result = await axios({
              method: 'get',
              url: `https://api.twitch.tv/helix/clips?broadcaster_id=${channelID}&first=3&started_at=${clipDate}&ended_at=${moment(Date.now()).toISOString()}`,
              headers: {
                'Client-ID': CLIENT_ID,
            ...
            <div className="clips-container">
                { clips && clips[0] ? <h2>{channelName}</h2> : null }
                {
                    clips ? clips.map(clip => (
                        <a href={clip.url} target="_blank" key={clip.id} rel="noopener noreferrer">
                            <div className="clip-wrapper">
                                <div className="channel-container-img">
                                    {
                                        clip.thumbnail_url ? <img src={clip.thumbnail_url} alt="Clip screen"/> : null
                                    }
                                </div>
